document.addEventListener("DOMContentLoaded", function () {
    ...
    const customPyramidPlugin = {
        ...
        beforeDraw(chart, args, options) {
            ...
            for (let i = 0; i < numLayers - 1; i++) {
                ...
                const dropOffPct = prevLayer.vol > 0 ? (((prevLayer.vol - currentLayer.vol) / prevLayer.vol) * 100).toFixed(1) : "0.0";
                ...
                if (options.hasComp) {
                    const compDropPct = prevLayer.compVol > 0 ? (((prevLayer.compVol - currentLayer.compVol) / prevLayer.compVol) * 100).toFixed(1) : "0.0";
            ...
            // 3. Draw Text In Center of Pyramid Slices
            for (let i = 0; i < numLayers; i++) {
                const y0 = pyTop + i * layerHeight;
                const y1 = pyTop + (i + 1) * layerHeight;
                const textY = (y0 + y1) / 2;
                ...
                if (options.hasComp) {
                    ...
                    ctx.fillText(layers[i].title, centerX, textY + 14);
                } else {
                    ctx.fillStyle = '#ffffff';
    ...
    if (ctxFunnel && legendContainer && canvasContainer) {
        ...
        if (hasComp && comp_imp > 0) {
            ...
            const isUp = diff >= 0;
            const colorCls = isUp ? 'text-[#10b981] bg-green-50' : 'text-error bg-red-50';
            const arrow = isUp ? '↑' : '↓';
            ...
            compOverallHTML = `
                <div class="mt-3 pt-3 border-t border-gray-100 w-full text-left">
                    ...
                    <div class="flex items-center gap-2 mb-0.5">
                        <span class="text-base font-bold text-gray-700 leading-none">${compRate}%</span>
                        <span class="text-[10px] font-semibold ${colorCls} px-1.5 py-0.5 rounded">${arrow} ${Math.abs(diff)}%</span>
                    </div>
                    <p class="text-[10px] text-gray-400">${comp_orders.toLocaleString()} orders</p>
                </div>
            `;
        }

        let leftColHTML = `
            <div class="bg-white border border-gray-100 shadow-[0_4px_12px_rgba(0,0,0,0.06)] rounded-2xl p-4 w-full shrink-0 max-w-sm flex flex-col items-start">
